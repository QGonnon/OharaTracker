<template>
  <Card class="manga-card bg-white dark:bg-slate-800 rounded-lg shadow-md overflow-hidden hover:shadow-xl transition-shadow duration-300 cursor-pointer max-w-xs">
    <!-- Image -->
    <template #header>
    ...
    <template #content>
      <div class="p-4">
        <div class="flex-grow flex flex-col justify-center text-center px-3">
          <p class="text-slate-600 dark:text-slate-300 text-sm truncate">{{ $t('cards.last_episode') }} {{ manga.lastEpisode || manga.lastChapter || '-' }}</p>
        </div>
      </div>
    </template>

    <template #footer>
      <div class="flex gap-3 justify-center mt-2 pb-3 px-4">
...
<script src="./AnimeCard.ts"></script>
