<template>
  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
    ...
    <div v-if="loading" class="flex flex-col items-center justify-center py-20">
      ...
      <p class="text-lg text-surface-500">Chargement de l'anime...</p>
    </div>
      ...
      <div class="lg:col-span-2 space-y-6">
          ...
          <template #content>
            <div class="space-y-4">
              ...
              <div class="flex items-start gap-3">
                ...
                <div>
                  <p class="text-sm text-surface-500 dark:text-surface-400">Dernier épisode</p>
                  <p class="text-base font-medium text-surface-900 dark:text-surface-0">
                    Épisode {{ manga.lastEpisode || manga.lastChapter || '-' }}
                  </p>
                </div>
              </div>
            </div>
        ...
        <div class="space-y-3">
          <Message v-if="addSuccess || isInLibrary" severity="success" :closable="false" icon="pi pi-check">
            {{ addSuccess ? 'Anime ajouté à votre bibliothèque.' : 'Déjà dans votre bibliothèque.' }}
          </Message>
          ...
          <Button
            ...
            :label="`Voir l'épisode ${manga.lastEpisode || manga.lastChapter}`"
            icon="pi pi-play"
            iconPos="left"
            severity="primary"
            size="large"
