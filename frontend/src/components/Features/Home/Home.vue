<template>
  ...
  <!-- ═══════════════════════════════════════════════
       HERO
  ════════════════════════════════════════════════ -->
  <section class="relative min-h-[calc(100vh-64px)] flex items-center bg-white dark:bg-slate-950 overflow-hidden">
    ...
    <div class="relative z-10 w-full max-w-7xl mx-auto px-6 py-20 flex flex-col lg:flex-row items-center gap-12 lg:gap-20">
      ...
      <div class="flex-1 max-w-xl">
        ...
        <h1 class="text-5xl lg:text-[3.4rem] font-extrabold leading-[1.1] text-slate-900 dark:text-white tracking-tight mb-5">
          {{ $t('home.heading_1') }}<br>
        ...
        <div class="grid grid-cols-2 gap-x-8 gap-y-3 pt-6 border-t border-slate-100 dark:border-white/5">
          <div v-for="stat in stats" :key="stat.label">
            <span class="text-lg font-bold text-slate-900 dark:text-white">{{ stat.value }}</span>
            <span class="text-xs text-slate-600 dark:text-slate-400 ml-1.5">{{ $t(stat.label) }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="flex-1 w-full max-w-md lg:max-w-none lg:flex-none lg:w-[460px]" aria-hidden="true">
        <div class="rounded-2xl overflow-hidden border border-slate-200 dark:border-white/10 shadow-2xl shadow-slate-200/60 dark:shadow-black/50 bg-white dark:bg-slate-900">
          <div class="flex items-center gap-1.5 px-4 py-3 bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-white/5">
            ...
            <div class="ml-3 flex-1 h-5 rounded bg-slate-200 dark:bg-slate-700 flex items-center px-2.5">
              <!-- zinc-400 sur zinc-700 ne donnait que 4,07:1 en thème sombre. -->
              <span class="text-[11px] text-slate-600 dark:text-slate-300">{{ $t('home.mock_url') }}</span>
            </div>
          </div>
          <div class="p-4">
            <div class="flex items-center justify-between mb-3 px-1">
              <span class="text-sm font-semibold text-slate-700 dark:text-slate-200">{{ $t('home.my_library') }}</span>
            ...
            <div class="space-y-0.5">
              <div v-for="item in mockItems" :key="item.title"
                ...
                <div class="flex-1 min-w-0">
                  <div class="text-sm font-medium text-slate-700 dark:text-slate-200 truncate group-hover:text-violet-600 dark:group-hover:text-violet-400 transition-colors">
                    {{ item.title }}
                  </div>
  ...
  ════════════════════════════════════════════════ -->
  <section id="screenshots" class="py-24 bg-slate-50 dark:bg-slate-900 border-t border-slate-100 dark:border-white/5">
    <div class="max-w-6xl mx-auto px-6">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-x-6 gap-y-12 items-start">
        ...
        <div class="flex flex-col gap-3">
          <div class="flex flex-col rounded-xl overflow-hidden border border-slate-200 dark:border-white/10 shadow-lg relative">
          <!-- Barre browser fictive -->
          <div class="flex items-center gap-1.5 px-3 py-2 bg-slate-50 dark:bg-slate-700 border-b border-slate-100 dark:border-white/5 flex-shrink-0">
          ...
          <div>
            <h3 class="font-semibold text-slate-900 dark:text-white text-sm mb-1">{{ $t('home.screenshot_library_title') }}</h3>
            <p class="text-slate-600 dark:text-slate-400 text-xs leading-relaxed">{{ $t('home.screenshot_library_desc') }}</p>
          </div>
        </div>
        ...
        <div class="flex flex-col gap-3">
          ...
          >
            <div class="flex items-center gap-1.5 px-3 py-2 bg-slate-50 dark:bg-slate-700 border-b border-slate-100 dark:border-white/5">
              <div class="w-2 h-2 rounded-full bg-red-400/70" />
              <div class="w-2 h-2 rounded-full bg-yellow-400/70" />
              <div class="w-2 h-2 rounded-full bg-green-400/70" />
            </div>
          ...
          <div>
            <h3 class="font-semibold text-slate-900 dark:text-white text-sm mb-1">{{ $t('home.screenshot_tracking_title') }}</h3>
            <p class="text-slate-600 dark:text-slate-400 text-xs leading-relaxed">{{ $t('home.screenshot_tracking_desc') }}</p>
          </div>
        </div>
        ...
        <div class="flex flex-col gap-3">
          <button
            type="button"
            class="block w-full text-left rounded-xl overflow-hidden border border-slate-200 dark:border-white/10 shadow-lg cursor-zoom-in focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-violet-600"
          ...
          <div>
            <h3 class="font-semibold text-slate-900 dark:text-white text-sm mb-1">{{ $t('home.screenshot_profile_title') }}</h3>
            <p class="text-slate-600 dark:text-slate-400 text-xs leading-relaxed">{{ $t('home.screenshot_profile_desc') }}</p>
          </div>
        </div>
  ...
  ════════════════════════════════════════════════ -->
  <section class="py-24 bg-white dark:bg-slate-950 border-t border-slate-100 dark:border-white/5">
    <div class="max-w-6xl mx-auto px-6">
      <div class="text-center mb-16">
        <span class="inline-block text-violet-600 dark:text-violet-400 text-sm font-semibold uppercase tracking-widest mb-3">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        <div v-for="feat in features" :key="feat.title"
             class="p-6 rounded-xl border border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-slate-900 hover:border-violet-200 dark:hover:border-violet-500/30 hover:shadow-md dark:hover:shadow-none transition-all duration-200 group">
          ...
          <h3 class="font-semibold text-slate-900 dark:text-white mb-2">{{ $t(feat.title) }}</h3>
          <p class="text-slate-500 dark:text-slate-400 text-sm leading-relaxed">{{ $t(feat.desc) }}</p>
        </div>
      </div>
    </div>
  </section>
  ...
  ════════════════════════════════════════════════ -->
  <section v-if="!isLoggedIn" class="py-24 bg-slate-50 dark:bg-slate-900 border-t border-slate-100 dark:border-white/5">
    <div class="max-w-3xl mx-auto px-6">
      <div class="text-center mb-14">
        <span class="inline-block text-violet-600 dark:text-violet-400 text-sm font-semibold uppercase tracking-widest mb-3">
          {{ $t('home.quickstart_label') }}
        </span>
        <h2 class="text-4xl font-bold text-slate-900 dark:text-white">{{ $t('home.quickstart_heading') }}</h2>
      </div>

      <div class="space-y-4 mb-6">
        <div v-for="(step, i) in steps" :key="i"
             class="flex items-start gap-5 p-5 rounded-xl bg-white dark:bg-slate-800/60 border border-slate-100 dark:border-white/5 hover:border-violet-200 dark:hover:border-violet-500/20 transition-colors">
          <div class="flex-shrink-0 w-9 h-9 rounded-full bg-violet-600 text-white flex items-center justify-center font-bold text-sm">
            {{ i + 1 }}
          </div>
          <div>
            <h3 class="font-semibold text-slate-900 dark:text-white mb-1">{{ $t(step.title) }}</h3>
            <p class="text-slate-500 dark:text-slate-400 text-sm leading-relaxed">{{ $t(step.text) }}</p>
          </div>
        </div>
      </div>
      ...
      <RouterLink :to="registerPath" class="block mt-8">
        <div class="group rounded-2xl border border-violet-200 dark:border-violet-500/25 bg-white dark:bg-slate-800/60 hover:border-violet-400 dark:hover:border-violet-500/50 hover:shadow-lg dark:hover:shadow-violet-500/5 transition-all duration-200 cursor-pointer px-8 py-7 text-center">
          <p class="font-bold text-slate-900 dark:text-white text-xl mb-2 group-hover:text-violet-600 dark:group-hover:text-violet-400 transition-colors">
            {{ $t('home.cta_heading') }}
          </p>
          <p class="text-slate-600 dark:text-slate-400 text-sm">{{ $t('home.footer') }}</p>
          <!-- Flèche décorative -->
  ...
  <!-- ═══════════════════════════════════════════════
       PREMIUM CTA
  ════════════════════════════════════════════════ -->
  <section class="relative py-28 overflow-hidden border-t border-slate-100 dark:border-white/5 bg-white dark:bg-slate-950">

    <div class="absolute inset-0 pointer-events-none">
      <div class="absolute -top-32 left-1/4 w-[520px] h-[520px] bg-violet-600/8 dark:bg-violet-600/15 rounded-full blur-3xl" />
      <div class="absolute -bottom-24 right-1/4 w-[420px] h-[420px] bg-violet-500/8 dark:bg-violet-500/12 rounded-full blur-3xl" />
    </div>

    <div class="relative z-10 max-w-5xl mx-auto px-6">
      <div class="rounded-3xl border border-slate-100 dark:border-white/8 bg-slate-50/80 dark:bg-slate-900/80 backdrop-blur-md shadow-xl dark:shadow-black/40 px-10 py-14 flex flex-col lg:flex-row items-center gap-10 lg:gap-16">

        <div class="flex-1 text-center lg:text-left">
          <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full border text-xs font-semibold tracking-wide mb-5"
              style="background: linear-gradient(90deg, rgba(124,58,237,0.08), rgba(249,115,22,0.08)); border-color: rgba(124,58,237,0.3);">
            <i class="pi pi-crown text-xs" style="color: #7c3aed"  aria-hidden="true"/>
            <span style="background: linear-gradient(90deg, #7c3aed, #f97316); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;">
              {{ $t('home.premium_badge') }}
            </span>
          </div>

          <h2 class="text-3xl lg:text-4xl font-extrabold tracking-tight mb-4 leading-tight"
              style="background: linear-gradient(90deg, #7c3aed, #f97316); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;">
            {{ $t('home.premium_heading') }}
          </h2>

          <p class="text-slate-500 dark:text-slate-400 text-base leading-relaxed mb-6 max-w-lg">
            {{ $t('home.premium_desc') }}
          </p>
        ...
        <div class="flex-shrink-0 flex flex-col items-center gap-4">
          <Button asChild v-slot="slotProps" size="large" class="px-8 font-semibold">
            <RouterLink
              ...
              style="background: linear-gradient(90deg, #7c3aed, #c2410c); border: none;"
            >{{ $t('home.premium_cta') }}</RouterLink>
          </Button>
          <p class="text-xs text-slate-600 dark:text-slate-400 text-center max-w-[14rem] leading-relaxed">
            {{ $t('home.premium_sub') }}
          </p>
        </div>
