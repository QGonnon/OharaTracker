    ...
    <div class="flex flex-col md:flex-row items-center gap-8 p-8 max-w-5xl w-full relative">
      ...
      <div class="flex flex-col gap-3 text-white max-w-xl">
        ...
        <div class="flex gap-3 mt-2">
          <RouterLink :to="workPath(spotlightItem)" class="inline-flex items-center gap-2 px-6 py-2 rounded-full bg-gradient-to-r from-violet-500 to-indigo-600 text-white font-bold shadow-lg hover:scale-105 transition">
  ...
  <div class="bg-gray-50 dark:bg-gray-900 min-h-[60vh] pb-16">
    ...
    <section v-if="trending.length && activeGenre === '' && activeType === 'all'" class="px-8 pt-10">
      ...
      <div class="flex gap-3 overflow-x-auto pb-2">
        <RouterLink v-for="(item, i) in trending" :key="item.title" :to="workPath(item)" class="flex items-center gap-3 min-w-[180px] max-w-[220px] bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-3 hover:shadow-lg transition">
          <span :class="['font-extrabold text-xl', Number(i) < 3 ? 'bg-gradient-to-r from-violet-500 to-pink-400 bg-clip-text text-transparent' : 'text-gray-600']">{{ String(Number(i) + 1).padStart(2, '0') }}</span>
    ...
    <section class="px-8 pt-10">
      <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        ...
        <div class="flex gap-2">
          <button type="button" :aria-pressed="sortBy === 'latest'" @click="sortBy = 'latest'" :class="['px-3 py-1 rounded-full text-xs font-semibold transition', sortBy === 'latest' ? 'bg-violet-600 text-white' : 'bg-gray-200 dark:bg-gray-800 text-gray-700 dark:text-gray-200 hover:bg-violet-100 dark:hover:bg-violet-900']">{{ $t('discovery.recent') }}</button>
          <button type="button" :aria-pressed="sortBy === 'alpha'" @click="sortBy = 'alpha'" :class="['px-3 py-1 rounded-full text-xs font-semibold transition', sortBy === 'alpha' ? 'bg-violet-600 text-white' : 'bg-gray-200 dark:bg-gray-800 text-gray-700 dark:text-gray-200 hover:bg-violet-100 dark:hover:bg-violet-900']">{{ $t('discovery.a_to_z') }}</button>
        </div>
      </div>
      <!-- Skeleton loader -->
      ...
      <div v-else class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2">
        <div v-for="item in filteredItems" :key="item.title" class="flex flex-col gap-4 my-2">
          <RouterLink :to="workPath(item)" class="relative aspect-[3/4] rounded-xl overflow-hidden bg-gray-100 dark:bg-gray-800 group block">
