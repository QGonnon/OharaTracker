<template>
  <Menu />
  <section class="min-h-[70vh] flex items-center bg-white dark:bg-slate-950 py-20">
    <div class="max-w-xl mx-auto px-6 text-center">
      ...
      <Button asChild v-slot="slotProps" class="font-semibold">
        <RouterLink :to="home" :class="slotProps.class">{{ $t('errors.not_found.back_home') }}</RouterLink>
      </Button>

      <nav class="mt-10 pt-8 border-t border-slate-100 dark:border-white/5">
        <p class="text-xs uppercase tracking-widest text-slate-600 dark:text-slate-400 mb-4">
          {{ $t('errors.not_found.suggestions') }}
        </p>
