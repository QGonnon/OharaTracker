<template>
  ...
  <section class="min-h-[70vh] bg-white dark:bg-zinc-950 py-20">
    <div class="max-w-3xl mx-auto px-6">
      ...
      <p v-if="loading" class="text-center text-gray-600 dark:text-zinc-300 text-sm">
        {{ $t('static.official_partners.loading') }}
      </p>

      <p v-else-if="!partners.length" class="text-center text-gray-600 dark:text-zinc-300 text-sm">
      ...
      <template v-else>
        ...
        <div v-if="highlighted.length" class="grid sm:grid-cols-2 gap-4 mb-8">
          ...
          >
            ...
            <span class="block font-bold text-gray-900 dark:text-white">{{ partner.name }}</span>
            <span v-if="partner.description" class="block text-sm text-gray-600 dark:text-zinc-300 mt-1">{{ partner.description }}</span>
          </a>
        </div>
        ...
        <div v-if="others.length" class="grid grid-cols-2 sm:grid-cols-3 gap-4">
          <a
            ...
            target="_blank"
            rel="noopener nofollow"
            class="p-5 rounded-xl border border-gray-100 dark:border-white/5 bg-gray-50 dark:bg-zinc-900 text-center font-semibold text-gray-700 dark:text-zinc-200 hover:border-violet-300 dark:hover:border-violet-700 transition"
          >
            {{ partner.name }}
          </a>
        </div>
      ...
      <p class="text-center text-sm mt-10 text-gray-600 dark:text-zinc-300">
